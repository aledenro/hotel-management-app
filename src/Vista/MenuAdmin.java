...
public class MenuAdmin extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        cboOpcion.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "1 - Listar todas las habitaciones disponibles por hotel ", "2 - Registrar reservación a una persona ", "3 - Eliminar reservación de una persona ", "4 - Eliminar todas las reservaciones por hotel  ", "5 - Buscar persona por número de cédula/pasaporte ", "6 - Validar disponibilidad de una habitación en un hotel ", "7 - Crear hotel" }));
    ...
    private void btnIrActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIrActionPerformed
        ...
        if (cboOpcion.getSelectedItem().toString().contains("1 - Listar todas las habitaciones disponibles por hotel")) {
            ListarHabitacionesDisponibles lhd = new ListarHabitacionesDisponibles();
            this.setVisible(false);
            lhd.setVisible(true);
        }
        if (cboOpcion.getSelectedItem().toString().contains("2 - Registrar reservación a una persona")) {
            ReservarAdmin ra = new ReservarAdmin();
            this.setVisible(false);
            ra.setVisible(true);
        }
        if (cboOpcion.getSelectedItem().toString().contains("3 - Eliminar reservación de una persona")) {
            EliminarReservasPersonaAdmin erpa = new EliminarReservasPersonaAdmin();
            this.setVisible(false);
            erpa.setVisible(true);
        }
        if (cboOpcion.getSelectedItem().toString().contains("4 - Eliminar todas las reservaciones por hotel")) {
            EliminarReservasHotel erh = new EliminarReservasHotel();
            this.setVisible(false);
            erh.setVisible(true);
        }
        if (cboOpcion.getSelectedItem().toString().contains("5 - Buscar persona por número de cédula/pasaporte")) {
            BuscarPersonaAdmin bpa = new BuscarPersonaAdmin();
            this.setVisible(false);
            bpa.setVisible(true);
        }
        if (cboOpcion.getSelectedItem().toString().contains("6 - Validar disponibilidad de una habitación en un hotel")) {
            ValidarDisponibilidadHabitacion vdh = new ValidarDisponibilidadHabitacion();
            this.setVisible(false);
            vdh.setVisible(true);
        }
        if (cboOpcion.getSelectedItem().toString().contains("7 - Crear hotel")) {
